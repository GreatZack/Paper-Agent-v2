...
def fresh_config():
    ...
    with patch.object(Config, "_instance", None), patch.object(Config, "_initialized", False):
        yield Config
...
"""
    ...
    assert cfg.get("default-model.api_key") == "sk-test-from-env"
    assert cfg.get("default-model.base_url") == "https://test.example.com/v1"


def test_config_keeps_unresolved_placeholder_when_env_missing(fresh_config, tmp_path, monkeypatch):
...
"""
    ...
    assert cfg.get("default-model.api_key") == "${MISSING_KEY}"
...
"""
    ...
    assert cfg.get("default-model.model") == "original-model"
